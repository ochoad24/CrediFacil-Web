...
<script lang="ts">
  ...
  import type { UpdateCompanyTypeRequest, CompanyType } from '$lib/types/companytype';
  import { loadingStore } from '$lib/stores/loading';
  import { toast } from '$lib/utils/toast';

  // Page data
  let companytypeId = $page.params.id;
  let form: UpdateCompanyTypeRequest = {};
  ...
  async function loadData() {
    loadingStore.show('Cargando CompanyType...');
    try {
  ...
  function validateForm(): boolean {
    ...
    // Estado - Optional field validation
    if (form.status?.trim() && form.status.trim().length > 255) {
      validationErrors.status = 'Estado debe tener máximo 255 caracteres';
    }
  ...
  async function handleSubmit() {
    ...
    loading = true;
    loadingStore.show('Actualizando CompanyType...');
    validationErrors = {};

    try {
      await companytypeService.update(companytypeId, form);
      success = true;
      toast.success('CompanyType actualizado exitosamente');
      goto('/companytypes');
    } catch (err: any) {
      console.error('Error updating companytype:', err);
      ...
      if (err.response?.data?.errors) {
        ...
        // Parse error message
        let errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'Error al actualizar companytype';
  ...
  <div class="max-w-2xl mx-auto">
    <div class="mb-6">
      <div class="flex items-center space-x-4">
        ...
        <div>
          <h1 class="text-2xl font-bold text-primary">Editar Tipo de empresa</h1>
    ...
    <div class="bg-surface shadow-sm rounded-lg border border-border">
      ...
      <div>
        <Input
          ...
          on:input={() => clearFieldError('status')}
        />
      </div>
      <div>
        <Input
