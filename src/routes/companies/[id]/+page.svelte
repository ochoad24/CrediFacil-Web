<script lang="ts">
	...
	async function loadCompany() {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Error al cargar el company';
		} finally {
			loading = false;
		}
	}
	...
	async function deleteCompany() {
		...
		const confirmed = await confirmation.danger(
			'¿Estás seguro de que quieres eliminar este company? Esta acción no se puede deshacer.',
			'Eliminar Company'
		);
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Error al eliminar company';
		}
	}
...
<svelte:head>
	<title>Detalle de Compañía - {PUBLIC_NAME_COMPANY}</title>
</svelte:head>
...
<div class="p-4 sm:p-6 bg-page min-h-screen">
		...
		<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 lg:pr-0 pr-16">
			<div class="flex items-center space-x-4">
				<button
					...
					title="Volver a companies"
					aria-label="Volver a companies"
				>
				...
				<div class="min-w-0 flex-1">
					<h1 class="text-xl sm:text-2xl font-bold text-primary truncate">
						{company ? company!.name : 'Detalle de Compañía'}
					</h1>
					<p class="text-sm sm:text-base text-secondary">Información completa del Compañía</p>
				</div>
			</div>
			...
			{#if company}
				...
				<div class="flex flex-col xs:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
					{#if authStore.hasPermission('companies.update')}
						<button
							on:click={() => goto(`/companies/${company!.id}/edit`)}
	...
	{#if loading}
		<div class="text-center py-8">
			...
			<p class="mt-2 text-secondary">Cargando Compañía...</p>
		</div>
	{:else if error}
				...
				<div class="bg-surface shadow-sm rounded-lg border border-border">
					<div class="p-6">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div>
								<h3 class="text-sm font-medium text-tertiary uppercase tracking-wide mb-2">Nombre legal</h3>
								<div class="space-y-3">
									<div class="flex items-center">
										<svg class="w-5 h-5 text-tertiary mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
										</svg>
										<span class="text-primary">{company.legal_name || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								<h3 class="text-sm font-medium text-tertiary uppercase tracking-wide mb-2">Identificación fiscal</h3>
								<div class="space-y-3">
									<div class="flex items-center">
										<svg class="w-5 h-5 text-tertiary mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
										</svg>
										<span class="text-primary">{company.tax_id || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.address || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.phone || '-'}</span>
									</div>
								</div>
							</div>
							<div>
							...
							<div>
								<h3 class="text-sm font-medium text-tertiary uppercase tracking-wide mb-2">Sitio web</h3>
								<div class="space-y-3">
									<div class="flex items-center">
										<svg class="w-5 h-5 text-tertiary mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
										</svg>
										<span class="text-primary">{company.website || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.is_active ? 'Sí' : 'No'}</span>
									</div>
								</div>
							</div>
							<div>
								<h3 class="text-sm font-medium text-tertiary uppercase tracking-wide mb-2">Fecha de establecimiento</h3>
								<div class="space-y-3">
									<div class="flex items-center">
										<svg class="w-5 h-5 text-tertiary mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
										</svg>
										<span class="text-primary">{company.established_date || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.notes || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.document_type_code || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.document_number || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.created_by_id || '-'}</span>
									</div>
								</div>
							</div>
							<div>
								...
								<div class="space-y-3">
									<div class="flex items-center">
										...
										<span class="text-primary">{company.updated_by_id || '-'}</span>
									</div>
								</div>
							</div>
			...
			<div class="space-y-6">
				<div class="bg-surface shadow-sm rounded-lg border border-border">
					<div class="p-6">
						...
						<div class="space-y-4">
							<div>
								<dt class="text-sm font-medium text-tertiary">ID del Compañía</dt>
	...
	{:else}
		<div class="text-center py-8">
			...
			<h3 class="mt-2 text-sm font-medium text-primary">Company no encontrado</h3>
			<p class="mt-1 text-sm text-tertiary">El company que buscas no existe o ha sido eliminado.</p>
			<div class="mt-6">
				<button
					on:click={handleBack}
