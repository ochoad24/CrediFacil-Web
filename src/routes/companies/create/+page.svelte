<!-- Auto-generated Create Page for Company -->
<script lang="ts">
  ...
  import type { CreateCompanyRequest } from '$lib/types/company';
  ...
  let form = {
    ...
    website: '' as string,
    is_active: false,
    established_date: '' as string,
    notes: '' as string,
  } satisfies CreateCompanyRequest as CreateCompanyRequest;
  ...
  async function handleSubmit() {
    ...
    loading = true;
    loadingStore.show('Creando Company...');
    validationErrors = {};

    try {
      await companyService.create(form);
      success = true;
      toast.success('Company creado exitosamente');
      goto('/companies');
    } catch (err: any) {
      ...
      if (err.response?.data?.errors) {
        ...
        // Parse error message
        let errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'Error al crear company';
...
<svelte:head>
  <title>Crear Compañía - {PUBLIC_NAME_COMPANY}</title>
</svelte:head>
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center space-x-4 lg:pr-0 pr-16">
        ...
        <div class="min-w-0 flex-1">
          <h1 class="text-xl sm:text-2xl font-bold text-primary truncate">Crear Compañía</h1>
