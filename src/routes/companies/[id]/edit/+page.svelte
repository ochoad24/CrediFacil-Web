<!-- Auto-generated Edit Page for Company -->
<script lang="ts">
  ...
  import type { UpdateCompanyRequest, Company } from '$lib/types/company';
  import { loadingStore } from '$lib/stores/loading';
  import { toast } from '$lib/utils/toast';

  // Page data
  let companyId = $page.params.id || '';
  let form: UpdateCompanyRequest = {};
  ...
  async function loadData() {
    ...
    loadingStore.show('Cargando Company...');
    try {
      ...
      documenttypeOptions = await companyService.getDocumentTypeOptions();
      userOptions = await companyService.getUserOptions();
  ...
  function validateForm(): boolean {
    ...
    // Nombre legal - Optional field validation
    if (form.legal_name?.trim() && form.legal_name.trim().length > 255) {
      validationErrors.legal_name = 'Nombre legal debe tener máximo 255 caracteres';
    }
    // Identificación fiscal - Optional field validation
    if (form.tax_id?.trim() && form.tax_id.trim().length > 255) {
      validationErrors.tax_id = 'Identificación fiscal debe tener máximo 255 caracteres';
    }
    ...
    // Sitio web - Optional field validation
    if (form.website?.trim() && form.website.trim().length > 255) {
      validationErrors.website = 'Sitio web debe tener máximo 255 caracteres';
    }
    // Está activo - Optional field validation
    // Fecha de establecimiento - Optional field validation
  ...
  async function handleSubmit() {
    ...
    loading = true;
    loadingStore.show('Actualizando Company...');
    validationErrors = {};

    try {
      await companyService.update(companyId, form);
      success = true;
      toast.success('Company actualizado exitosamente');
      goto('/companies');
    } catch (err: any) {
      ...
      if (err.response?.data?.errors) {
        ...
        // Parse error message
        let errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'Error al actualizar company';
        ...
        if (errorMessage.includes('duplicate key') || errorMessage.includes('SQLSTATE 23505')) {
          ...
          if (errorMessage.includes('idx_companies_name') || errorMessage.includes('name')) {
            errorMessage = 'Ya existe un company con ese nombre';
          } else if (errorMessage.includes('idx_companies_')) {
            // Generic duplicate error
            errorMessage = 'Ya existe un company con esos datos';
          } else {
...
<svelte:head>
  <title>Editar Compañía - {PUBLIC_NAME_COMPANY}</title>
</svelte:head>
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center space-x-4 lg:pr-0 pr-16">
        ...
        <div class="min-w-0 flex-1">
          <h1 class="text-xl sm:text-2xl font-bold text-primary truncate">Editar Compañía</h1>
    ...
    <div class="bg-surface shadow-sm rounded-lg border border-border">
      ...
      <div>
        <Input
          ...
          label="Nombre legal"
          placeholder="Ingresa nombre legal"
          required={ false }
          
          tabindex={ 2 }
          error={validationErrors.legal_name}
          on:input={() => clearFieldError('legal_name')}
        />
      </div>
      <div>
        <Input
          ...
          placeholder="Ingresa identificación fiscal"
          required={ false }
          
          tabindex={ 3 }
          error={validationErrors.tax_id}
          on:input={() => clearFieldError('tax_id')}
        />
      </div>
      <div>
        <Input
      ...
      <div>
        <Input
          id="established_date"
          type="datetime-local"
          bind:value={form.established_date}
          label="Fecha de establecimiento"
          placeholder="Ingresa fecha de establecimiento"
          required={ false }
          
          tabindex={ 9 }
          error={validationErrors.established_date}
          on:input={() => clearFieldError('established_date')}
        />
      </div>
      <div>
        <Input
