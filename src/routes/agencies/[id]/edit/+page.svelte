<!-- Auto-generated Edit Page for Agency -->
<script lang="ts">
  ...
  import type { UpdateAgencyRequest, Agency } from '$lib/types/agency';
  import { loadingStore } from '$lib/stores/loading';
  import { toast } from '$lib/utils/toast';

  // Page data
  let agencyId = $page.params.id || '';
  let form: UpdateAgencyRequest = {};
  ...
  async function loadData() {
    ...
    loadingStore.show('Cargando Agency...');
    try {
  ...
  async function handleSubmit() {
    ...
    loading = true;
    loadingStore.show('Actualizando Agency...');
    validationErrors = {};

    try {
      await agencyService.update(agencyId, form);
      success = true;
      toast.success('Agency actualizado exitosamente');
      goto('/agencies');
    } catch (err: any) {
      ...
      if (err.response?.data?.errors) {
        ...
        // Parse error message
        let errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'Error al actualizar agency';
...
<svelte:head>
  <title>Editar Agencia - {PUBLIC_NAME_COMPANY}</title>
</svelte:head>
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center space-x-4 lg:pr-0 pr-16">
        ...
        <div class="min-w-0 flex-1">
          <h1 class="text-xl sm:text-2xl font-bold text-primary truncate">Editar Agencia</h1>
