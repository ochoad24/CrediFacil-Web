<!-- Auto-generated List Page for Agency -->
<script lang="ts">
	...
	import type { Agency, AgencySearchParams } from '$lib/types/agency';
	...
	async function loadAgencies(
		...
		try {
			const params: AgencySearchParams = {
				page,
				limit,
			...
			agencies = Array.isArray(response.data) ? response.data : [];
			pagination = {
			...
			error = err instanceof Error ? err.message : 'Error al cargar agencias';
	...
	async function deleteAgency(id: string, name: string) {
		...
		try {
			...
			await loadAgencies(pagination.page, pagination.limit, searchTerm, sortKey, sortDirection);
		} catch (err) {
			error = err instanceof Error ? err.message : 'Error al eliminar agencia';
		}
	}
	...
	function handleSearch(event: CustomEvent<{ search: string }>) {
		...
		loadAgencies(1, pagination.limit, searchTerm, sortKey, sortDirection);
	}

	function handlePageChange(event: CustomEvent<{ page: number }>) {
		loadAgencies(event.detail.page, pagination.limit, searchTerm, sortKey, sortDirection);
	}

	function handleLimitChange(event: CustomEvent<{ limit: number }>) {
		loadAgencies(1, event.detail.limit, searchTerm, sortKey, sortDirection);
	}
	...
	function handleSort(event: CustomEvent<{ key: string; direction: SortDirection }>) {
		...
		loadAgencies(pagination.page, pagination.limit, searchTerm, sortKey, sortDirection);
	}
...
<svelte:head>
  <title>Agencias - {PUBLIC_NAME_COMPANY}</title>
</svelte:head>
...
<div class="p-4 sm:p-6">
	<div class="mb-6">
		...
		<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 lg:pr-0 pr-16">
			<div class="min-w-0 flex-1">
				<h1 class="text-xl sm:text-2xl font-bold text-primary">Agencias</h1>
				<p class="text-sm sm:text-base text-secondary">Gestiona los agencias del sistema</p>
			</div>
			<PermissionGuard permission="agencies.create">
				<Button
					on:click={() => goto('/agencies/create')}
					variant="primary"
					size="md"
  ...
  <DataTable
		...
		{sortKey}
		{sortDirection}
		searchPlaceholder="Buscar agencias... (presiona Enter)"
		emptyMessage="No se encontraron agencias. Comienza creando un nuevo agencia."
	...
	>
		<svelte:fragment slot="actions" let:item>
			<div class="flex justify-end space-x-1">
				<button
					on:click={() => goto(`/agencies/${(item as unknown as Agency).id}`)}
					class="btn-icon btn-icon-neutral"
					title="Ver detalles"
					aria-label="Ver detalles del agencia"
				>
				...
				{#if authStore.hasPermission('agencies.update')}
					<button
						on:click={() => goto(`/agencies/${(item as unknown as Agency).id}/edit`)}
				...
				{#if authStore.hasPermission('agencies.delete')}
					<button
						on:click={() => deleteAgency((item as unknown as Agency).id, (item as unknown as Agency).name)}
